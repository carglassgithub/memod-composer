...
class EditorLinkCard extends HTMLElement {
  constructor(){
    ...
    wrapper.setAttribute('class', `editor-link-card ${imageUrl ? '' : 'no-image'}`);
    ...
    removeImage.onclick = (e) => { this._onRemove(e) }
  ...
  sanitizeAttributes(name){
    return this.getAttribute(name) === 'undefined' ? undefined : this.getAttribute(name);
  }
}
